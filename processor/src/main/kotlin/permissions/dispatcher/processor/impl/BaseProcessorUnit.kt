...
public abstract class BaseProcessorUnit : ProcessorUnit {
    ...
    private fun addResultCaseBody(builder: MethodSpec.Builder, needsMethod: ExecutableElement, rpe: RuntimePermissionsElement, targetParam: String, grantResultsParam: String) {
        ...
        if (hasDenied && !ADD_WITH_CHECK_BODY_MAP.containsKey(needsPermissionParameter)) {
            builder.beginControlFlow("if (\$T.VERSION.SDK_INT < \$T.VERSION_CODES.M && !\$T.hasSelfPermissions(\$N, \$N))",
                    BUILD_CLASS, BUILD_CLASS, PERMISSION_UTILS, getActivityName(targetParam), permissionFieldName(needsMethod))
